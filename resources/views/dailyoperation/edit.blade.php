<x-app-layout>
    <x-slot name="header">
        ...
        <div class="row">
            ...
            <div class="col-md-1">
                <a href="{{ route('index-dailyoperations') }}" class="btn btn-secondary">       
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="py-4 px-4 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('update-dailyoperation', $dailyoperation->id) }}" method="POST">
                    ...
                    @method('PUT')
                    <div class="py-2">
                        <x-label for="name" :value="__('التشغيل')" />
        
                        <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="$dailyoperation->name" required autofocus />
                    </div>
                    ...
                    <div class="py-2">
                        ...
                        <select class="form-control" name="status">
                            <option value="active" @if($dailyoperation->status == 'active') selected @endif >مفعلة</option>
                            <option value="disabled" @if($dailyoperation->status == 'disabled') selected @endif >غير مفعلة</option>
                        </select>                    
                    </div>
